...
public class ProjectsPreparedApp {
	...
	public static void main(String[] args) {
		...
		try {
			for (int listingIndex = 0; listingIndex < listings.size(); listingIndex++) {
				...
				StringBuilder builder = new StringBuilder();
				builder.append("INSERT INTO listing (room_id, price) VALUES (?, ?)");
				...
				stmt = conn.prepareCall(builder.toString());
				stmt.setLong(1, list.getRoomId());
				stmt.setDouble(2, list.getPrice());
		...
		try {
			...
			stmt.setString(4, guest.getAddress1());
			stmt.setString(5, guest.getAddress2());
			stmt.setString(6, guest.getCity());
			stmt.setString(7, guest.getZip());
			stmt.setString(8, guest.getPhone());
			System.out.println("After : " + stmt.toString());
		...
		Guest newGuest = null;
		try {
			stmt = conn.prepareCall("SELECT * FROM guest WHERE email = ?");
